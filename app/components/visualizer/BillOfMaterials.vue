  ...
  <div class="flex flex-col h-full">
    ...
    <div v-if="!bom.length" class="flex-1 flex flex-col items-center justify-center gap-3 px-6 py-10 text-center">
      ...
      <div>
        ...
        <p class="text-xs text-text-secondary mt-1">Selecciona superficies y aplica productos para generar el presupuesto.</p>
      </div>
    </div>
    ...
    <template v-else>
      ...
      <div class="px-4 pt-4">
        ...
        >
          ...
          <Icon
            :name="showDims ? 'lucide:chevron-up' : 'lucide:chevron-down'"
            ...
            <div class="grid grid-cols-3 gap-2">
              <div v-for="field in ['width', 'length', 'height'] as const" :key="field">
                ...
                <div class="relative">
                  ...
                  />
                  <span class="absolute right-2 top-1/2 -translate-y-1/2 text-xs text-text-tertiary">m</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="flex-1 overflow-y-auto overscroll-contain scrollbar-hide px-4 py-3">
        <div class="space-y-2.5">
          <div
            v-for="item in bom"
            :key="item.productId + item.surfaceId"
          ...
          >
            <div class="flex items-center gap-3 px-3 py-2.5">
              ...
              <div class="w-10 h-10 rounded-lg overflow-hidden flex-shrink-0 bg-surface-100">
                <img
                  :src="item.product.images.texture || item.product.images.thumbnail"
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center gap-1.5 mt-0.5">
                  <span class="inline-flex items-center gap-1 text-xs text-text-secondary bg-black/[0.05] px-1.5 py-0.5 rounded-full">
            ...
            <!-- Stats row -->
            <div class="grid grid-cols-3 divide-x divide-black/[0.06] border-t border-black/[0.06] bg-black/[0.02]">
              ...
              <div class="px-3 py-1.5 text-center">
                ...
                <p class="text-sm font-semibold text-primary">{{ formatPrice(item.totalPrice, item.currency) }}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="px-4 pb-4 space-y-2 border-t border-black/[0.06] pt-3">
        ...
        <p class="text-xs text-text-tertiary">
          * Incluye {{ Math.round(wasteFactor * 100) }}% de desperdicio. Precio referencial — consulta al proveedor.
        </p>
      </div>
    </template>
  </div>
