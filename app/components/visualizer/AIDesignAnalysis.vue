...
<template>
  <div class="flex flex-col h-full px-4 py-4 gap-4 overflow-y-auto overscroll-contain scrollbar-hide">
    <template v-if="generatedImage">
      ...
      >
        <Icon
          :name="isAnalyzing ? 'lucide:loader-2' : 'lucide:brain-circuit'"
      ...
      <template v-if="aiAnalysis">
        ...
        <div class="rounded-xl border border-black/[0.08] bg-white p-4 space-y-2">
          ...
          <div class="h-2 rounded-full bg-black/[0.07] overflow-hidden">
            <div
              :class="['h-full rounded-full transition-all duration-700', scoreBarColor]"
        ...
        <div class="grid grid-cols-1 gap-2">
          <div
            v-for="(value, key) in { 'Armonía cromática': aiAnalysis.colorHarmony, 'Coherencia estilística': aiAnalysis.styleMatch, 'Cobertura de materiales': aiAnalysis.estimatedCoverage }"
            :key="key"
            class="rounded-xl border border-black/[0.08] bg-white p-3"
          >
